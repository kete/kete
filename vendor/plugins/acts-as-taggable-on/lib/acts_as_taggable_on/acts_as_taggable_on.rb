          ...
          (custom_contexts + self.class.tag_types.map(&:to_s)).each do |tag_type|
            ...
            self.class.transaction do
              ...
              new_tag_names.each do |new_tag_name|
                ...
                # Additionally, only create if the tagging doesn't exist
                existing_tagging = Tagging.first(:conditions => {
                  :tag_id => new_tag.id, :taggable_type => self.class.name, :taggable_id => self.id
                })
                Tagging.create(:tag_id => new_tag.id, :context => tag_type,
                               :taggable => self, :tagger => owner,
                               :basket_id => self.basket_id) unless existing_tagging
              end
            end
          end
