module OAI::Provider::Response
  class RecordResponse < Base
    ...
    def header_for(record)
      ...
      if record.respond_to?(:complete_header) && !deleted?(record)
        @builder.target! << record.complete_header
      else
    ...
    # Walter McGinnis, 2008-06-15
    # return header and data for record
    # based on configuration
    def header_and_data_for(record, response_builder)
      # test if the resulting records already have full record preformatted
      # if not lay them out by hand
      complete_method_name = 'to_complete_' + options[:metadata_prefix] unless options[:metadata_prefix].nil?
      if !complete_method_name.nil? && record.respond_to?(complete_method_name.to_sym) && !deleted?(record)
        @builder.target! << record.send(complete_method_name)
      else
        response_builder.record do
          header_for record
