          ...
          # TODO: it looks like this still needs multiple support?
            ...
            if extended_field.multiple
              ...
              multiple_values.each do |m_field_value|
                ...
                params[zoom_class_for_params]['extended_content_values'][extended_field.label_for_params][m_field_count] = { :value => m_field_value.to_s.strip, :circa => circa }
                m_field_count += 1
              end
            else
              circa = value =~ /(circa|c.?\d+)/ # circa 2010, c 2010, c.2010
              value = (value =~ /\d+/ && $1) if circa
              params[zoom_class_for_params]['extended_content_values'][extended_field.label_for_params] = { :value => value.to_s.strip, :circa => circa }
            end
