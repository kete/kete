...
module ExtendedContentTestUnitHelper
  ...
  def test_xml_attributes_without_position
    ...
    model.update_attribute(:extended_content_xml, '<some_tag>something</some_tag>')
    
    assert_valid model
    
    assert_equal '<some_tag>something</some_tag>', model.extended_content_xml 
  ...
  def test_xml_attributes_without_position_without_data
    ...
    assert_equal '', model.extended_content_xml
    assert model.xml_attributes_without_position.empty?
  end
  
  def test_extended_content
    model = Module.class_eval(@base_class).create!(new_model_attributes)    
    model.update_attribute(:extended_content_xml, '<some_tag xml_element_name="dc:something">something</some_tag>')
    
    assert_valid model
    
    assert_equal '<some_tag xml_element_name="dc:something">something</some_tag>', model.extended_content_xml 
    assert_equal({ "xml_element_name" => "dc:something", "value" => "something" }, model.extended_content['some_tag'])
  end
  
  def test_extended_content_with_multiple_nodes
    model = Module.class_eval(@base_class).create!(new_model_attributes)    
    model.update_attribute(:extended_content_xml, '<some_tag xml_element_name="dc:something">something</some_tag><some_other_tag xml_element_name="dc:something_else">something_else</some_other_tag>')
    
    assert_valid model
    
    assert_equal '<some_tag xml_element_name="dc:something">something</some_tag><some_other_tag xml_element_name="dc:something_else">something_else</some_other_tag>', model.extended_content_xml 
    assert_equal 2, model.extended_content.size
    assert_equal({ "xml_element_name" => "dc:something", "value" => "something" }, model.extended_content['some_tag'])
    assert_equal({ "xml_element_name" => "dc:something_else", "value" => "something_else" }, model.extended_content['some_other_tag'])
  end
  ...
  def test_extended_content_without_data
    ...
    assert_equal '', model.extended_content_xml 
    assert model.extended_content.empty?
  end
  ...
  def test_extended_content_setter_with_undefined_field
    model = Module.class_eval(@base_class).create!(new_model_attributes)
    model.extended_content = { "some_tag" => "value" }
    ...
    assert_nil model.extended_content["some_tag"]
  end
