...
module ApplicationHelper
  ...
  def display_search_sources
    ...
    SearchSource.all.each do |source|
      search_source_id = source.title.gsub(/\W/, '_').downcase
      search_source_url = source.base_url + @item.title.escape_for_url
      entries = Feed.fetch_entries(search_source_url)
      html += content_tag('h4', h(source.title), :id => "#{search_source_id}_heading")

      html += "<span class='search_source_more'>( "
      html += "top #{source.limit} results"
      html += " | " + link_to('more &raquo;', search_source_url) if entries.size > source.limit
      html += ") </span>"

      html += "<ul id='#{search_source_id}_items'>"
      images = Array.new
      entries[0..(source.limit - 1)].each do |entry|
        if !entry.media_thumbnail.nil? || !entry.enclosure.nil?
          images << entry
        else
          title = entry.summary ? truncate(strip_tags(entry.summary).squish, :length => 300, :omission => '...') : ''
          html += "<li>" + link_to(h(entry.title), h(entry.url), :title => title) + "</li>"
        end
      end
      html += '</ul>'
      images = images.collect do |image|
        img_div = "<div class='image'>"
        img = if !image.media_thumbnail.nil?
          image_tag(image.media_thumbnail, :alt => "#{h(image.title)}. ", :title => "#{h(image.title)}. ", :width => 50, :height => 50)
        else
          image_tag(image.enclosure, :alt => "#{h(image.title)}. ", :title => "#{h(image.title)}. ", :width => 50, :height => 50)
        end
        title = image.summary ? truncate(strip_tags(image.summary).squish, :length => 300, :omission => '...') : ''
        img_div += link_to(img + h(image.title), h(image.url), :title => title)
        img_div += '</div>'
        img_div += "<div style='clear:both;'></div>"
        img_div
      end
      html += "<div id='#{search_source_id}_images' class='images'>#{images}</div>" unless images.blank?
    end
    return html if html.blank?
    "<div id='search_sources'>" +
      #render("topics/secondary_wrapper_start", :div_id => 'search_sources_wrapper', :class_suffix => nil) +
