module MembersHelper
  ...
  def admin_actions_correct_list_item(user, action_key, action_label, tool_count)
    ...
    when 'destroy'
      ...
      if user_contributions == 0
        html_string = li_with_correct_class(tool_count) + link_to( action_label, { :action => :destroy, :id => user, :authenticity_token => form_authenticity_token }, :confirm => t('members_helper.admin_actions_correct_list_item.are_you_sure'), :method => :post ) + '</li>'
      else
    ...
    when 'ban'
      if !user.banned_at.nil?
        ...
        html_string = li_with_correct_class(tool_count) + link_to( action_label, { :action => action_key, :id => user, :authenticity_token => form_authenticity_token }, :confirm => t('members_helper.admin_actions_correct_list_item.are_you_sure'), :method => :post ) + '</li>'
      end
    else
      html_string = li_with_correct_class(tool_count) + link_to( action_label, { :action => action_key, :id => user, :authenticity_token => form_authenticity_token }, :confirm => t('members_helper.admin_actions_correct_list_item.are_you_sure'), :method => :post ) + '</li>'
    end
  ...
  def get_user_role_creation_date_for(user, role_type, basket)
    begin
      ...
      t('members_helper.get_user_role_creation_date_for.unknown')
    end
  end
end
